...
def DALJ2(T,rho):
    DALJ = ((+2.01546797 + rho * (-28.17881636 +rho * (+28.28313847 + rho * (-10.42402873))))
            +(-19.58371655 + rho * (75.62340289 + rho * ((-120.70586598) +rho * (93.92740328 + rho * (-27.37737354))))) / math.sqrt(T)
            +((29.34470520 + rho * ((-112.35356937) +rho * (+170.64908980 + rho * ((-123.06669187)
            +rho * 34.42288969))))+(-13.37031968 + rho * (65.38059570 + rho * ((-115.09233113) + rho * (88.91973082
...
def ULJ(T, rho):
    ...
    eta=PI/6.0 *rho*d**3
    sum1 = 2.01546797+rho*((-28.17881636)+rho*(+28.28313847+rho*(-10.42402873)))
    sum2 = -19.58371655*1.5+rho*(75.62340289*1.5+rho*((-120.70586598)*1.5+rho*(93.92740328*1.5+rho*(-27.37737354)*1.5)))
    sum2 = sum2/math.sqrt(T)
    sum3 = 29.34470520*2.+rho*(-112.35356937*2.+rho*(170.64908980*2.+rho*( -123.06669187*2.+rho*34.42288969*2.)))
    sum4 = -13.37031968*3.+rho*( 65.38059570*3.+rho*(-115.09233113*3.+rho*( 88.91973082*3.+rho*(-25.62099890)*3.)))
    sum4 = sum4/T
...
def PressureLJ(T, rho):
    ...
    sum2 = -19.58371655*2
    sum2 = sum2 + rho*(75.62340289*3+rho*(-120.70586598*4+rho*(93.92740328*5+rho*(-27.37737354)*6)))
    sum2 = sum2/math.sqrt(T)
    sum3 = 29.34470520*2.+rho*((-112.35356937)*3.+rho*( +170.64908980*4.+rho*( (-123.06669187)*5.+rho* 34.42288969*6.)))
    sum4 = -13.37031968*2.+rho*(65.38059570*3.+rho*(-115.09233113*4.+rho*(88.91973082*5.+rho*(-25.62099890)*6.)))
    sum4 = sum4/T
    sum5 =(sum1+ sum2 + (sum3+sum4)/T)*rho**2
    PLJ = ((zHS(eta)+BC(T)/math.exp(gammaBH(T)*rho**2)*rho*(1.0-2*gammaBH(T)*rho**2))*T+sum5)*rho
    return PLJ
