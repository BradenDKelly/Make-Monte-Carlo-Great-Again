...
def potential_1 ( ri, di, box, r, d ):
    ...
    sr2_ovr       = 1.77*100 # Overlap threshold (pot > 100)
    ...
    if fast:
        ...
        for a in range(na):
            for b in range(na):
                rab      = rij + di[a,:] - d[:,b,:] # All atom-atom vectors for given a and b // atom b in all molecules
...
"""No bullshit quaternions in the energy loop. Keep it clean. Keep it fast."""
def potential_2(ai, ri, box, at, r):
    ...
    nj, d = r.shape
    assert d == 3, 'Dimension error for r in potential_2'
    assert ri.size == 3, 'Dimension error for ri in potential_2'
    ak, dj = at.shape
    assert dj == 3, ' Dimension error for a in potential_2'
   ...
   # assert nna==na and ndim==3, 'Dimension error for di'
    ...
    sr2_ovr       = 1.77*10 # Overlap threshold (pot > 100)
    rm_cut_box    = ( r_cut + diameter ) / box # Molecular cutoff in box=1 units
    rm_cut_box_sq = rm_cut_box**2              # squared
    assert rm_cut_box<0.5, 'rm_cut/box too large'
    r_cut_sq = r_cut ** 2
    ...
    if fast:
        ...
        rij = rij - np.rint(rij) # Periodic boundary conditions in box=1 units
        rij = rij * box          # Now in sigma=1 units
        rij = np.repeat(rij, np.ones((len(rij),), dtype=int)*3, axis=0)
        #ai = ai * box
        #at = at * box
        for a in range(na):
            #for b in range(na):
            rab      = ai[a,:] - at   # rij + di[a,:] - d[:,b,:] # All atom-atom vectors for given a and b
            rab = rab - np.rint(rab/box) * box # Periodic boundary conditions in box=1 units
            #rab = np.minimum(rab, box-rab)
            rab_sq   = np.sum(rab**2,axis=1)    # Squared separations
            in_range = rab_sq < r_cut_sq        # Set flags for within cutoff
            sr2      = 1.0 / rab_sq             # (sigma/rab)**2
            ovr      = sr2 > sr2_ovr            # Set flags for any overlaps
    ...
    partial.pot = partial.pot  # 4*epsilon
    partial.vir = partial.vir / 3.0  # 24*epsilon and divide virial by 3
